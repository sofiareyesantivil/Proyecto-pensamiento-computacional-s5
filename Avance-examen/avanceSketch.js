...
function resetAnimacion() {
  ...
  // 🔥 CORRECCIÓN: Solo reseteamos el frame si el GIF ya está totalmente cargado
  if (gif1 && typeof gif1.setFrame === 'function') {
    gif1.setFrame(0);
  }
  if (gif2 && typeof gif2.setFrame === 'function') {
    gif2.setFrame(0);
  }
...
function draw() {
  ...
  // FLECHA
  if (gifActual === gif2) {
    // 🔥 CORRECCIÓN: Validar que los métodos existan antes de llamarlos
    if (typeof gif2.getCurrentFrame === 'function' && typeof gif2.numFrames === 'function') {
...
function mousePressed() {
  ...
  if (iniciado && gifActual !== gif2) {
    gifActual = gif2;
    // 🔥 CORRECCIÓN: Validación adicional al cambiar de GIF
    if (gif2 && typeof gif2.setFrame === 'function') {
      gif2.setFrame(0);
    }
    return;
